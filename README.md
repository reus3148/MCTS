...
23. **v1.7**: MCTS를 쓰는 유일한 이유 — 관찰한 것에 **적응**하는 능력 — 을 통째로
    ...
    **문제는 알고리즘이 아니라 환경입니다.**
24. **v1.8**: v1.7의 처방대로 **적응할 자리**를 하나 열었습니다 — 재발 시 구제치료
    결정(v0.6 환경, `configs/dynamic_v0_6.json`). 결정 지점의 값은 **+0.0009**(z = 0.93,
    MDE 0.0028 = 격차의 14%)로 검출되지 않았고, 그 값조차 폐루프가 아니었습니다:
    **우리가 선언한 구제치료가 5년 지평 안에서 순손실**이라(가이드라인 정책이 강제로 받아
    **−0.0010, z = −13.7**) MCTS는 결정의 85%를 거절했고 **재발 3년차부터는 100% 거절**
    했습니다. **v1.5의 비대칭을 새 채널에서 재생산**했지만, 사전 예측 "지평 인공물"이 같은
    실행에서 잡았습니다. "적응할 자리"의 조건이 셋으로 구체화됐습니다 — **자주 열리고,
    선택이 갈리고, 지평이 결과를 자르지 않아야** 합니다. 다음 과제는 결정 지점 추가가
    아니라 **지평 처리**입니다.
25. **v1.9**: 지평을 고쳤습니다 — 결정 지평 뒤에 **말기 가치 10년**(v0.7 환경,
    `configs/dynamic_v0_7.json`; GENIE BPC 진행 후 생존 중앙 3.9년·5년 40%·10년 18%가 근거).
    v1.8의 진단이 맞았습니다: 재발 연도별 거절률 **62→100%가 1.5→1.4%** 로 평평해졌고,
    **같은 파라미터**의 구제치료가 가이드라인 정책에게 순손실(−0.0010)에서 **순이득
    (+0.0050, z = 14.4)** 으로 바뀌었습니다. 그런데 결정 지점의 값은 여전히 검출되지 않았고
    (+0.0059, z = 1.15, MDE 27%) 이번엔 **"항상 치료"** 로 붕괴했습니다(수용 98.7%). 그리고
    말기 가치가 **v1.5의 비대칭을 증폭**했습니다 — MCTS만 쓰는 강화 항암이 **12% → 39%**.
    영대조는 절대 허용치를 넘겨 형식상 실패했고(z = −1.58) 규약을 상대값으로 바꿨습니다.
    **v0.7의 격차는 인용하지 않습니다**(척도 3배).
26. **v2.0**: 구제 결정이 **환자에 따라 갈리는** 위험비가 있는지, 탐색 없이 환경의 산술로
    지도를 그렸습니다(40명 × 재발 4연도 × 위험비 21). **없었습니다** — 치료가 이득인 칸이
    0.94까지 100%, 0.98부터 0%, **환자별 무차별 위험비 0.95~0.96**(퍼짐 0.01). 지도가 고른
    0.96에서 시드 24로 탐색을 돌리자 **정답조차 재현하지 못했습니다**(오라클 1년차 100% 대
    MCTS 56%, 네 해 모두 동전 던지기) — 두 선택의 차이 0.002가 예산 1024의 잡음 0.02 아래라서.
    결정 지점의 값 +0.0002(격차의 0.4%, z = 0.06). **결정 지점의 값 = 판돈 × 상태 의존 ×
    해상도**이고, 곱셈 이득 × 고정 비용 채널은 셋을 동시에 못 줍니다. 구제 선은 여기서
    접고, 다음은 **표준치료 이득 선언**과 **효과 이질성 선언**(사람 검토)입니다.
...
[결정 지점 v1.8 보고서](reports/decision-points-v1.8/README.md),
[지평 v1.9 보고서](reports/horizon-v1.9/README.md),
[무차별 v2.0 보고서](reports/indifference-v2.0/README.md),
...
reports/decision-points-v1.8/
  tables/                 팔별 격차·시드별 결과·행동 구성·재발 연도별 거절률
reports/horizon-v1.9/
  tables/                 위와 같음 + GENIE 진행 후 생존
reports/indifference-v2.0/
  tables/                 무차별 지도(40×4×21)·환자별 무차별 위험비·연도별 오라클 대 MCTS
configs/dynamic_v0_6.json v0.5 + 재발 시 구제 결정 (5년 지평에서 순손실, 인용 금지)
configs/dynamic_v0_7.json v0.6 + 말기 가치 10년 (이후 환경의 기본; 절대 격차 인용 금지)
...
analysis/dynamic/salvage.py  구제 결정 위임 환경·영대조·지평 진단 (v1.8)
analysis/genie/survival.py   GENIE 진행 후 생존 KM (v1.9)
analysis/dynamic/indifference.py 결정론적 잔여 효용·무차별 지도 (v2.0)
...
- **새 채널을 열 때는 두 진단을 먼저 봅니다**(v1.8): (a) 가이드라인 정책이 그 채널로
  손해를 보는가 — 본다면 그 채널은 v1.5와 같은 비대칭이다; (b) 탐색 정책의 선택이 지평
  잔여에 단조인가 — 그렇다면 값은 지평 인공물이다. v0.6의 구제치료는 둘 다 걸렸습니다
  (NCCN −0.0010, z = −13.7; 거절률 62% → 100%). **v0.6 환경의 격차는 인용하지 않습니다.**
- **결정 지점은 판돈·상태 의존·해상도가 다 있어야 값을 가집니다**(v2.0). 새 결정 지점을
  열 때는 탐색 전에 환경의 산술로 지도를 그려 셋이 있는지 확인합니다. 무차별 지점의 탐색
  결과는 잡음이라 인용하지 않습니다 — 정답을 아는 문제에서 44%p 빗나갔습니다.
- **v0.7 환경의 절대 격차는 v0.5와 비교하지 않습니다**(v1.9). 말기 가치로 효용 최대치가
  1.0 → 3.0이 됐고, 꼬리가 MCTS 전용 채널(강화 항암 12% → 39%)을 증폭했습니다. 상대 격차
  (격차 ÷ NCCN 효용)로만 비교합니다: 2.47% → 2.75%.
- **허용치와 검정력은 격차 대비 상대값으로 등록합니다**(v1.9). 절대값으로 등록한 영대조
  허용치가 척도 변화를 따라가지 못해 형식상 실패했습니다.
- **5년 지평은 추적 중 결정을 구조적으로 무가치하게 만듭니다**(v1.8). v0.7의 말기 가치가
  이것을 고쳤습니다(v1.9) — 다만 꼬리는 "결정 없는 10년"이라 비관적이고, 재발 후 사망 위험
  배수(×2.0)는 GENIE 대비 낙관적입니다. 이득은 지평 안
  남은 해에 비례하고 비용은 즉시 들기 때문에, 지평 또는 말기 가치를 고치기 전에는 어떤
  후기 결정도 같은 인공물을 안습니다.
